# iqdb-persist -- Engineering Directives
...
iqdb-persist is what moves iQDB from demo-only to actually usable: it serializes indexes and vectors to disk, logs mutations to a WAL, and recovers cleanly after a crash. It is the embedded persistence layer and is designed to sit on the `storage-io` substrate (the renamed `fsys-rs`) rather than touching files directly.
...
- All saves are atomic via temp-then-rename; a partial write must never corrupt existing data.
- WAL replay is the recovery contract: every mutation is logged before it is applied in memory.
...
- `iqdb-index`: wraps any index as persistable
- `storage-io`: disk I/O substrate (integration pending the fsys-rs rename)
